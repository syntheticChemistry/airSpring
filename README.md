...
| Phase 3.9: NUCLEUS primal | **51 capabilities** | airSpring biomeOS primal, 51 capabilities (science + provenance + composition + cross-spring + inference), JSON-RPC incl. **`primal.announce`** (Wave 17) |
...
*May 16, 2026 — **Wave 17 Signal Adoption**: `primal.announce` replaces 3-call registration (with legacy fallback); `nest.store` + `nest.commit` signal dispatch wired into provenance pipeline (biomeOS manages graph composition, graceful fallback to legacy); `primal.info` introspection handler; **51 capabilities** (was 49); registry synced to 451-method canonical (Wave 17); L5 certification evolved to `validate_primal_announce`. **1,057 lib + 62 forge tests**, 0 clippy. May 14: Tower triple-first (bearDog + songBird + skunkBat) per upstream plasmidBin manifest.
