...
fn validate_gpu_parity(
    ...
    for tc in tests.as_array().expect("array") {
        ...
        let cpu_result = cpu_batcher
            .compute_gpu(&[station])
            .expect("CPU compute");
        let gpu_result = gpu_batcher
            .compute_gpu(&[station])
            .expect("GPU compute");
        ...
        let ea = actual_vapour_pressure_rh(
            station.tmin, station.tmax, station.rh_min, station.rh_max,
        );
...
fn benchmark_throughput(
    ...
    // Build seasonal batch: 365 × 4 stations = 1,460 station-days
    ...
    for st in stations {
        ...
        for doy in 1..=365_u32 {
            all_days.push(StationDay {
                tmax: seasonal_value(doy, tmax_range[0].as_f64().unwrap(), tmax_range[1].as_f64().unwrap()),
                tmin: seasonal_value(doy, tmin_range[0].as_f64().unwrap(), tmin_range[1].as_f64().unwrap()),
                rh_max: seasonal_value(doy, rh_max_range[0].as_f64().unwrap(), rh_max_range[1].as_f64().unwrap()),
                rh_min: seasonal_value(doy, rh_min_range[0].as_f64().unwrap(), rh_min_range[1].as_f64().unwrap()),
                wind_2m: json_field(st, "wind_2m"),
                rs: seasonal_value(doy, rs_range[0].as_f64().unwrap(), rs_range[1].as_f64().unwrap()),
    ...
    let gpu_per_iter = gpu_elapsed / gpu_iters;

    // Benchmark CPU (5 iterations)
    ...
    let cpu_per_iter = cpu_elapsed / cpu_iters;
    ...
    v.check_bool(
        ...
        gpu_result.as_ref().is_some_and(|r| r.et0_values.len() == n),
    );
    ...
    v.check_bool(
        ...
        gpu_result
            .as_ref()
            .is_some_and(|r| r.backend_used == Backend::Gpu),
    );

    // Parity check: GPU seasonal totals match CPU within tolerance
    let gpu_vals = &gpu_result.as_ref().unwrap().et0_values;
    let cpu_vals = &cpu_result.as_ref().unwrap().et0_values;
    ...
    v.check_bool(
        "Seasonal total GPU ≈ CPU (< 1%)",
        pct_diff < 1.0,
    );

    // Per-day max divergence
    ...
    v.check_bool(
        "Max daily GPU-CPU divergence < 0.1 mm/day",
        max_diff < 0.1,
    );
}

fn validate_batch_scaling_gpu(
    v: &mut ValidationHarness,
    device: &Arc<WgpuDevice>,
) {
    ...
    let gpu_batcher =
        BatchedEt0::gpu(Arc::clone(device)).expect("BatchedEt0::gpu()");
    ...
    for sz in [10, 100, 1000, 10_000] {
        ...
        let result = gpu_batcher
            .compute_gpu(&batch)
            .expect("batch GPU compute");
        ...
        println!("  N={sz:>6}: {:.0} µs, max_diff={max_diff:.2e}", elapsed.as_micros());
...
fn main() {
    ...
    let parity_bm =
        parse_benchmark_json(PARITY_JSON).expect("parity benchmark must parse");
    let seasonal_bm =
        parse_benchmark_json(SEASONAL_JSON).expect("seasonal benchmark must parse");
