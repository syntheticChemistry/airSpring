...
const BENCHMARK_JSON: &str =
    include_str!("../../../control/gamma_cdf/benchmark_gamma_cdf.json");
...
fn validate_numeric_checks(v: &mut ValidationHarness, bench: &Value) {
    ...
    for c in checks {
        ...
        match name {
            "exponential_x0.5" => {
                let p = GammaParams { alpha: 1.0, beta: 1.0 };
            ...
            "chi2_df2_x2" => {
                ...
                let p = GammaParams { alpha: 1.0, beta: 2.0 };
                let obs = gamma_cdf(2.0, &p);
                let expected = json_f64(c, &["expected"]).expect("expected");
                v.check_abs(name, obs, expected, tolerance_or_default(c, 1e-10));
            }
            "gamma_2_1_at_1" => {
                let p = GammaParams { alpha: 2.0, beta: 1.0 };
                let obs = gamma_cdf(1.0, &p);
                let expected = json_f64(c, &["expected"]).expect("expected");
                v.check_abs(name, obs, expected, tolerance_or_default(c, 1e-10));
            }
...
fn validate_range_checks(v: &mut ValidationHarness, bench: &Value) {
    ...
    let p1 = GammaParams { alpha: 1.0, beta: 1.0 };
    let p21 = GammaParams { alpha: 2.0, beta: 1.0 };
    v.check_abs("reference exponentialCDF(1)", gamma_cdf(1.0, &p1), expo, 1e-10);
    v.check_abs("reference gamma(2,1)CDF(1)", gamma_cdf(1.0, &p21), gamma21, 1e-10);
}
